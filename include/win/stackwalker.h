...
extern "C" {
    ...
    SW_API_DECL sw_context* sw_create_context_capture(uint32_t options, const sw_callbacks* callbacks, void* userptr);
    SW_API_DECL sw_context* sw_create_context_capture_other(uint32_t options, uint32_t process_id,
        sw_sys_handle process, const sw_callbacks *callbacks, void* userptr);
    SW_API_DECL sw_context* sw_create_context_exception(uint32_t options,
        sw_exception_pointers exp_ptrs,
        const sw_callbacks* callbacks, void* userptr);
    SW_API_DECL sw_context* sw_create_context_catch(uint32_t options, const sw_callbacks* callbacks, void* userptr);
...
_SW_PRIVATE bool sw__load_modules_internal(sw_context_internal* ctxi, HANDLE process, DWORD process_id)
{
    if (!sw__get_module_list_TH32(ctxi, process_id)) {
        return ctxi->fEnumerateLoadedModules64(process, (PENUMLOADED_MODULES_CALLBACK64)sw__enum_loaded_modules_cb, ctxi) ? true : false;
    }
    return true;
}
...
_SW_PRIVATE sw_context* sw__create_context(uint32_t options, uint32_t process_id, sw_sys_handle process,
    PCONTEXT context_win, const sw_callbacks *callbacks, void* userptr)
{
    SW_ASSERT(process);
...
SW_API_IMPL sw_context* sw_create_context_capture(uint32_t options, const sw_callbacks *callbacks, void* userptr)
{
...
SW_API_IMPL sw_context* sw_create_context_capture_other(uint32_t options, uint32_t process_id,
    sw_sys_handle process, const sw_callbacks* callbacks,
    void* userptr)
{
...
SW_API_IMPL sw_context* sw_create_context_catch(uint32_t options, const sw_callbacks* callbacks, void* userptr)
{
